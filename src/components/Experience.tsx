...
import { experience, research, profile } from '../data/content'
import { Highlight } from '../lib/highlight'
import type { Role, Research as ResearchT } from '../data/content'

function RoleItem({ item, index }: { item: Role; index: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <h3 className="font-semibold text-[var(--color-text)]">
          {item.title} · <span className="text-[var(--color-accent)]">{item.company}</span>
        </h3>
...
function ResearchItem({ item, index }: { item: ResearchT; index: number }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 14 }}
      whileInView={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <h3 className="font-semibold text-[var(--color-text)]">
          {item.title} · <span className="text-[var(--color-accent)]">{item.org}</span>
        </h3>
        <span className="font-mono text-xs text-[var(--color-text-faint)]">{item.period}</span>
      </div>
    ...
    <Section id="experience" index="02." title="Experience">
      ...
      <div>
        {experience.map((role, i) => (
          <RoleItem key={role.company} item={role} index={i} />
        ))}
      </div>
      ...
      <div>
        {research.map((role, i) => (
          <ResearchItem key={role.org} item={role} index={i} />
        ))}
      </div>
    </Section>
  )
